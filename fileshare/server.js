...
// 多文本共享：textshare/index.json + textshare/{id}.txt（旧版 shared-text.txt 仅在首次初始化时迁移）
const textShareDir = path.join(__dirname, 'textshare');
const textShareIndexFile = path.join(textShareDir, 'index.json');
const legacySharedTextFile = path.join(__dirname, 'shared-text.txt');
...
async function readTextShareIndex() {
  try {
    const raw = await fs.readFile(textShareIndexFile, 'utf8');
...
async function writeTextShareIndex(notes) {
  await fs.ensureDir(textShareDir);
  await fs.writeFile(textShareIndexFile, JSON.stringify({ notes }, null, 2), 'utf8');
}

async function readNoteText(id) {
  if (!isValidNoteId(id)) return '';
  const p = path.join(textShareDir, `${id}.txt`);
  if (!(await fs.pathExists(p))) return '';
  return fs.readFile(p, 'utf8');
}

async function writeNoteText(id, text) {
  if (!isValidNoteId(id)) throw new Error('invalid note id');
  await fs.ensureDir(textShareDir);
  await fs.writeFile(path.join(textShareDir, `${id}.txt`), text, 'utf8');
}

async function initTextShare() {
  await fs.ensureDir(textShareDir);
  let notes = await readTextShareIndex();
  if (notes.length === 0) {
    let initialText = '';
    if (await fs.pathExists(legacySharedTextFile)) {
      try {
        initialText = await fs.readFile(legacySharedTextFile, 'utf8');
      } catch (e) {
        console.warn('读取旧版 shared-text.txt 失败:', e.message);
      }
    }
    const id = newNoteId();
    notes = [{ id, title: '默认笔记', updatedAt: new Date().toISOString() }];
    await writeTextShareIndex(notes);
    await writeNoteText(id, initialText);
    console.log('文本共享已初始化（多笔记），已迁移旧数据（如有）');
  }
}
...
// 兼容旧客户端：第一篇笔记
app.get('/api/shared-text', checkPassword, async (req, res) => {
  try {
    const notes = await readTextShareIndex();
    const first = notes[0];
    const text = first ? await readNoteText(first.id) : '';
    res.json({ text });
  } catch (error) {
    res.status(500).json({ error: '读取共享文本失败' });
  }
});
...
app.post('/api/shared-text', checkPassword, async (req, res) => {
  try {
    ...
    const first = notes[0];
    if (!first) {
      return res.status(500).json({ error: '笔记数据异常' });
    }
    await writeNoteText(first.id, text);
    first.updatedAt = new Date().toISOString();
    await writeTextShareIndex(notes);
    res.json({ message: '文本更新成功', text });
  } catch (error) {
...
// 多文本：列表（不含正文）
app.get('/api/shared-texts', checkPassword, async (req, res) => {
  try {
    const notes = await readTextShareIndex();
    res.json({
      notes: notes.map((n) => ({
...
// 单篇：正文 + 标题
app.get('/api/shared-texts/:id', checkPassword, async (req, res) => {
  try {
...
// 新建笔记
app.post('/api/shared-texts', checkPassword, async (req, res) => {
  try {
    ...
    if (text !== undefined && typeof text !== 'string') {
      return res.status(400).json({ error: '无效的正文' });
    }
    title = (title || '新笔记').trim().slice(0, 200) || '新笔记';
    text = typeof text === 'string' ? text : '';
    const id = newNoteId();
    const now = new Date().toISOString();
    const notes = await readTextShareIndex();
    notes.push({ id, title, updatedAt: now });
    await writeTextShareIndex(notes);
    await writeNoteText(id, text);
...
// 更新笔记（标题、正文可单独或同时提交）
app.post('/api/shared-texts/:id', checkPassword, async (req, res) => {
  try {
    const id = req.params.id;
    if (!isValidNoteId(id)) {
      return res.status(400).json({ error: '无效的笔记 ID' });
    ...
    if (title !== undefined) {
      ...
      notes[idx].title = title.trim().slice(0, 200) || '未命名';
    }
    if (text !== undefined) {
      if (typeof text !== 'string') {
        return res.status(400).json({ error: '无效的正文' });
    ...
    await writeTextShareIndex(notes);
    const bodyText = text !== undefined ? text : await readNoteText(id);
    res.json({
      message: '保存成功',
      id,
...
// 删除笔记（至少保留一篇）
app.delete('/api/shared-texts/:id', checkPassword, async (req, res) => {
  try {
    const id = req.params.id;
    if (!isValidNoteId(id)) {
      return res.status(400).json({ error: '无效的笔记 ID' });
    ...
    const notes = await readTextShareIndex();
    if (notes.length <= 1) {
      return res.status(400).json({ error: '至少保留一篇笔记' });
    }
    const next = notes.filter((n) => n.id !== id);
    if (next.length === notes.length) {
      return res.status(404).json({ error: '笔记不存在' });
    }
    await writeTextShareIndex(next);
    const p = path.join(textShareDir, `${id}.txt`);
    try {
      await fs.remove(p);
    } catch (e) {
